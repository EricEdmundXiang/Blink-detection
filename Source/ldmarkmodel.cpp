...
/*	cv::Rect mFaceRects;
	...
	for (int i = 0; i < (pResults ? *pResults : 0); i++)
	{
		...
		mFaceRects.x = p[0];
		mFaceRects.y = p[1];
		mFaceRects.width = p[2];
		mFaceRects.height = p[3];
	}
	cv::Rect maxRect = mFaceRects;
	x = mFaceRects.x;
	y = mFaceRects.y;
	w = mFaceRects.width;
	h = mFaceRects.height;
...
int ldmarkmodel::track(const cv::Mat& src, cv::Mat& current_shape, bool isDetFace){
	...
	cv::Rect mfaceBox = faceBox & cv::Rect(0, 0, src.cols, src.rows);
...
//    }
//	static int faceNum;
	float maxArea = 0;//记录最大面积的facebox
...
//			cv::Rect mFaceRects;
			...
			for (int i = 0; i < (pResults ? *pResults : 0); i++)
			{
				...
				mFaceRects.push_back(cv::Rect(p[0], p[1], p[2], p[3]));
			}
			if (mFaceRects.size() <= 0) {
				current_shape = cv::Mat();
				return SDM_ERROR_FACENO;
